...
try {
    ...
    if ($PSVersionTable.PSVersion.Major -ge 6) {
        $response = Invoke-WebRequest -Uri "https://portainer-server.lan:9444" -SkipCertificateCheck -TimeoutSec 5 -UseBasicParsing
    } else {
        # PowerShell 5.1 workaround
        $originalCallback = [System.Net.ServicePointManager]::ServerCertificateValidationCallback
        [System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}
        $response = Invoke-WebRequest -Uri "https://portainer-server.lan:9444" -TimeoutSec 5 -UseBasicParsing
        [System.Net.ServicePointManager]::ServerCertificateValidationCallback = $originalCallback
    }
    if ($response.StatusCode -eq 200) {
        Write-Host "  ✓ Portainer UI reachable at https://portainer-server.lan:9444" -ForegroundColor Green
        $passedChecks++
    } else {
...
try {
    $tcpTest = Test-NetConnection -ComputerName portainer-server.lan -Port 8000 -InformationLevel Quiet
    if ($tcpTest) {
